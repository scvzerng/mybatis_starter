...
/**        a        =       b   AND
 *单个条件 field operator value logic
    ...
    private Object converteValue(Object source){
        if(this.operator==Operator.IN||this.operator==Operator.BETWEEN){
            if(source instanceof String){
                ...
                throw new RuntimeException(String.format("error value from field:%s type:%s value:%s ",column.getColumn(),column.getJavaType().getSimpleName(),JSON.toJSONString(value)));
            }
